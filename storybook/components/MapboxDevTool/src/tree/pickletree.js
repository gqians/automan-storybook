...
class PickleTree {
   ...
   */
	constructor(obj) {
		...
		// node edit callback
		this.nodeEdit = obj.nodeEditCallback;
		// tree json data
   ...
   */
	clearDebris() {
		...
		const elms = document.querySelectorAll('.drag_triggered');
		for (let i = 0; i < elms.length; i++) {
   ...
   */
	orderNode(e) {
		const isBefore = Number(e.target.dataset.target) === 1;
		...
		if (target !== null) {
			...
			if (typeof this.orderCallback === 'function') this.orderCallback(mainNode, targetNode, isBefore);
		}
	}
	/**
   ...
   */
	toggleNode(node) {
		if (node.childs.length > 0) {
			const ie = document.getElementById('i_' + node.id);
			const ule = document.getElementById('c_' + node.id);
   ...
   */
	 ...
	 */
	editNode(node, text) {
		this.updateNodeTitle(node, text);
		if (this.nodeEdit !== undefined) this.nodeEdit(node, text);
	}
	/**
		...
		const childCheck = async(node) => {
			...
			if (node.childs.length > 0) {
				...
				for (let i = 0; i < node.childs.length; i++) {
					const c_node = this.getNode(node.childs[i].split('node_')[1]);
   ...
   */
	createNode(obj) {
		...
		const node = {
			...
			checkStatus: false,
			// edit status for node
			editable: false,
			// node order botton status
			showOrder: false,
   ...
   */
	...
	updateNodeTitle(node, title) {
		const element = document.querySelector(`#${node.id} span`);
		element.innerText = title;
		node.title = title;
		node.data.n_title = title;
	}
	/**
   *
   ...
   */
	drawNode(node) {
		...
		// node i item
		const i_item = document.createElement('i');
		// node ul item
		...
		if (this.config.order) {
			...
			if (node.showOrder) {
				...
				up_i.classList.add('fa', 'fa-arrow-up', 'fa-arrow-disabled');
				dw_i.classList.add('fa', 'fa-arrow-down', 'fa-arrow-disabled');
				o_div.appendChild(up_i);
				o_div.appendChild(dw_i);
			}
		...
		// set a_item title
		// a_item.innerHTML += ` <span id="edit_${node.id}" autocorrect="off" autocomplete="off" contentEditable=${node.editable}>` + node.title + "</span>";
	...
	setNodeEvents(node, parent) {
		// editNode(node)
		document.getElementById('edit_' + node.id).addEventListener('keypress', (e) => {
			if (event.keyCode === 13) {
				e.preventDefault();
				this.editNode(node, document.getElementById('edit_' + node.id).innerText);
			}
		});
   ...
   */
	drawData() {
		...
		if (this.data.length > 0) {
			// first reshape data
			const order = (list, p = { n_id: 0, Child: [] }, tree = []) => {
				const childrens = list.filter((y) => y.n_parentid === p.n_id);
			...
			const set = (list) => {
				for (let i = 0; i < list.length; i++) {
					this.createNode({
						...
						n_editable: list[i].n_editable,
						n_showOrder: list[i].n_showOrder,
						n_showSwitch: list[i].n_showSwitch,
					});
					if (list[i].Child) {
						set(list[i].Child);
					}
