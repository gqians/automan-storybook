...
const setDevElement = () => {
	...
	<div id="mapbox-dev-tool" ontouchstart=${dragStart} onmousedown=${dragStart} ontouchmove=${drag} onmousemove=${drag}  ontouchend=${dragEnd} onmouseup=${dragEnd} style="pointer-events:all;width:400px;height:600px;background-color:white;display:flex;flex-direction: column;">
		<div style="flex: 1 1 auto;display:flex;align-items: center;justify-content: center;box-shadow: 0px 4px 6px 1px rgba(20.19, 19.85, 19.85, 0.25);" x-data="{ menus: ['map', 'layers', 'sources', 'markers', 'tools'] }">
			<template x-for="type in menus">
				<span x-text="type" style="max-width:80px;display:inline-block;flex:1 1 auto;text-align:center;">
				</span>
			</template>
		</div>
